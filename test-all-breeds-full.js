...
// Все 100 пород из Dialogflow (уникальные значения)
const breeds = [
  ...
  "Селкирк-рекс длинношерстный", "Ориентальная длинношерстная"
];

// Убираем дубликаты
const uniqueBreeds = [...new Set(breeds)];

console.log(`📊 Всего пород в списке: ${breeds.length}`);
console.log(`📊 Уникальных пород: ${uniqueBreeds.length}\n`);
...
async function testAllBreeds() {
  console.log(`🧪 Тестирование ${uniqueBreeds.length} уникальных пород...\n`);
  ...
  for (let i = 0; i < uniqueBreeds.length; i++) {
    const breed = uniqueBreeds[i];
    process.stdout.write(`\r[${i + 1}/${uniqueBreeds.length}] Проверяю: ${breed}...`);
  ...
  console.log(`✅ Найдено на сайте: ${results.found.length}/${uniqueBreeds.length} (${Math.round(results.found.length/uniqueBreeds.length*100)}%)`);
  console.log(`❌ Не найдено: ${results.notFound.length}/${uniqueBreeds.length} (${Math.round(results.notFound.length/uniqueBreeds.length*100)}%)`);
  console.log(`\n📋 Детали:`);
  ...
  if (results.notFound.length > 0) {
    ...
    if (results.notFound.length > 30) {
      console.log(`   ... и еще ${results.notFound.length - 30} пород`);
    }
  }
